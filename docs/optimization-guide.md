...
### 第 9 轮（2026-05-16）— 最终审查

| 编号     | 项目                   | 变更文件                                       | 状态 |
|--------|----------------------|--------------------------------------------|----|
| **新增** | Dockerfile Rust 版本校正 | `Dockerfile`（1.84→1.94，添加依赖缓存优化）           | ✅  |
| **新增** | .dockerignore 优化     | `.dockerignore`（排除 docs/deploy/.env）       | ✅  |
| **新增** | versioning 模块注册      | `server/mod.rs`（添加 `pub mod versioning`）   | ✅  |
| **新增** | auth_service 测试扩展    | `auth_service.rs`（3 个测试：未找到账号 + 密码错误 + 禁用） | ✅  |
| **新增** | 已知问题文档               | 本页新增"已知剩余问题"章节                             | ✅  |
...
| 编号 | 问题                        | 影响                                                   | 建议                                  |
|----|---------------------------|------------------------------------------------------|-------------------------------------|
| 1  | 优雅关闭使用 `process::exit(0)` | Salvo 0.93 `Server` 无 `stop()` 方法，`select!` 退出后主进程结束 | 升级 Salvo 0.94+ 可使用 `server.stop()`  |
| 2  | JWT 测试依赖全局配置              | `default_jwt()` 需 `conf::get()`，单元测试中无法调用            | 将 `Principal` 编码拆出为纯函数，注入 `&JWT` 参数 |
| 3  | `page_query_params!` 宏    | IDE 无法展开，调试困难                                        | 可替换为 `#[derive(PageQuery)]` 派生宏     |
| 4  | 无集成测试                     | 只有 Service 层单元测试，无 HTTP 级集成测试                        | 引入 `testcontainers` + `reqwest`     |

### 配置层面

| 编号 | 问题             | 影响                       | 建议                                  |
|----|----------------|--------------------------|-------------------------------------|
| 5  | 配置无热重载         | 修改 YAML 需重启进程            | 使用 `config` crate 的 `watch` feature |
| 6  | 无配置加密          | 数据库密码明文在 YAML 中          | 生产环境通过 `APP_` 环境变量覆盖                |
| 7  | CORS 配置仅支持静态列表 | 不支持动态 origin（如根据请求头动态允许） | 使用 `AllowOrigin::dynamic()`         |

### 功能层面

| 编号 | 问题             | 影响                    | 建议                                        |
|----|----------------|-----------------------|-------------------------------------------|
| 8  | 限流仅全局级别        | 无法按 IP/用户/Token 分别限流  | 扩展 `RateLimitMiddleware` 支持 keyed limiter |
| 9  | Token 黑名单在内存中  | 重启后失效，不支持多实例          | 迁移到 Redis                                 |
| 10 | 缓存仅内存级别        | 重启后丢失，不支持多实例          | 迁移到 Redis / memcached                     |
| 11 | 无分布式追踪         | 跨服务请求无法串联             | 集成 OpenTelemetry + Jaeger                 |
| 12 | API 版本仅辅助函数    | 路由前缀需手动添加 `/v1/`      | 可扩展为自动版本路由                                |
| 13 | 无 WebSocket 支持 | 无法实现实时推送              | 集成 `tokio-tungstenite`                    |
| 14 | 测试覆盖率低         | 仅 3 个 auth_service 测试 | 逐步补充 sys_user_service + handler 测试        |
...
```
已实现: 55+ 项优化（8 轮 + 1 轮审查）
P0-P3 完成率: 39/43 ✅ 91%
新增文件: 34 个
修改文件: 55+ 个
单元测试: 3 个 (auth_service: 未找到/密码错误/账号禁用)
```

*优化工程完成。项目已从"单体脚手架"进化为"具备生产就绪特征的可测试微服务平台"。*
